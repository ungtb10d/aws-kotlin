...
import aws.sdk.kotlin.services.s3.model.ObjectPart
...
internal fun deserializeGetObjectAttributesPartsDocument(deserializer: Deserializer): GetObjectAttributesParts {
    ...
    val TOTALPARTSCOUNT_DESCRIPTOR = SdkFieldDescriptor(SerialKind.Integer, XmlSerialName("PartsCount"))
    val OBJ_DESCRIPTOR = SdkObjectDescriptor.build {
        trait(XmlSerialName("GetObjectAttributesParts"))
        ...
        field(TOTALPARTSCOUNT_DESCRIPTOR)
    }
            ...
            when (findNextFieldIndex()) {
                ...
                PARTS_DESCRIPTOR.index -> builder.parts =
                    deserializer.deserializeList(PARTS_DESCRIPTOR) {
                        val col0 = mutableListOf<ObjectPart>()
                        while (hasNextElement()) {
                            val el0 = if (nextHasValue()) { deserializeObjectPartDocument(deserializer) } else { deserializeNull(); continue }
                            col0.add(el0)
                        }
                        col0
                    }
                TOTALPARTSCOUNT_DESCRIPTOR.index -> builder.totalPartsCount = deserializeInt()
                null -> break@loop
                else -> skipValue()
            }
        }
